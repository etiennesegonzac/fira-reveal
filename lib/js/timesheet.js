...
(function() {
  ...
  Timesheet.prototype.insertData = function() {
    ...
    for (var n = 0, m = this.data.length; n < m; n++) {
      ...
      var line = [
        '<span style="margin-left: ' + bubble.getStartOffset() + 'px; width: ' + bubble.getWidth() + 'px;" class="bubble bubble-' + (cur.type || 'default') + '" data-duration="' + (cur.end ? cur.end.getFullYear() - cur.start.getFullYear() : '') + '"></span>',
  ...
  Timesheet.prototype.drawSections = function() {
    ...
    for (var c = this.year.min; c <= this.year.max; c++) {
      html.push('<section>' + c + (c > 0 ? '00' : '') + '</section>');
    }
